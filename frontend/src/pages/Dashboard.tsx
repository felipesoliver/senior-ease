...
const Dashboard = () => {
  ...
  const advanceStep = () => {
    setSteps((prev) => {
      const nextIndex = prev.findIndex((s) => !s.completed);
      if (nextIndex === -1) return prev;
      const updated = prev.map((s, i) =>
        i === nextIndex ? { ...s, completed: true } : s
      );
      const isLast = nextIndex === prev.length - 1;
      if (isLast) {
        toast.success("Parabéns! Você completou todas as etapas! 🎊");
      } else {
        toast.success(`"${prev[nextIndex].title}" concluído!`);
      }
      return updated;
    });
  };
        ...
        <Card className="border-2">
          ...
          <CardContent className="space-y-3">
            {reminders.map((r) => (
              ...
              >
                <r.icon className="h-7 w-7 shrink-0 text-primary" aria-hidden />
                <p className="text-lg font-medium text-foreground">{r.text}</p>
              </div>
            ))}
          </CardContent>
        </Card>
