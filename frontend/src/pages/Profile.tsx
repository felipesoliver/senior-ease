...
import { usePreferences, FontSize, ContrastLevel, NavigationMode } from "@/contexts/PreferencesContext";
      ...
      <header className="sticky top-0 z-10 border-b-2 border-border bg-card shadow-sm">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" size="sm" className="gap-2 text-base" aria-label="Sair">
      ...
      <main className="mx-auto max-w-4xl space-y-6 px-4 py-6 pb-12">
        ...
        <Card className="border-2">
          <CardContent className="flex items-center gap-4 p-6">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-primary/10 border-2 border-primary">
            ...
            <div className="grid grid-cols-3 gap-3">
              {fontSizeOptions.map((opt) => (
                <button
                  key={opt.value}
                  onClick={() => {
                    updatePreference("fontSize", opt.value);
                    toast.success(`Fonte alterada para ${opt.label}`);
                  }}
                ...
                >
                  ...
                  <span className="text-sm font-medium text-muted-foreground">{opt.label}</span>
                </button>
              ))}
            </div>
          </CardContent>
            ...
            <div className="grid grid-cols-2 gap-3">
              {contrastOptions.map((opt) => (
                <button
                  key={opt.value}
                  onClick={() => {
                    updatePreference("contrast", opt.value);
                    toast.success(`Contraste: ${opt.label}`);
                  }}
                  className={`flex items-center justify-center gap-2 rounded-xl border-2 p-4 text-lg font-semibold transition-all focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/40 ${
                    preferences.contrast === opt.value
                      ? "border-primary bg-primary-light text-foreground"
                      : "border-border bg-card text-muted-foreground hover:border-primary/40"
                  }`}
                  aria-pressed={preferences.contrast === opt.value}
                >
                  {opt.label}
                </button>
              ))}
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {navigationOptions.map((opt) => (
                <button
                  key={opt.value}
                  onClick={() => {
                    updatePreference("navigationMode", opt.value);
                    toast.success(`Navegação: ${opt.label}`);
                  }}
                  className={`flex flex-col items-start gap-1 rounded-xl border-2 p-4 text-left transition-all focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/40 ${
                    preferences.navigationMode === opt.value
                      ? "border-primary bg-primary-light"
                      : "border-border bg-card hover:border-primary/40"
                  }`}
                  aria-pressed={preferences.navigationMode === opt.value}
                >
          ...
          <CardContent className="space-y-1">
            ...
            <div className="flex items-center justify-between rounded-xl border-2 border-border p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <Label htmlFor="extra-confirm" className="text-lg font-medium text-foreground cursor-pointer">
                    Confirmações extras
                  </Label>
              ...
              <Switch
                ...
                onCheckedChange={(v) => updatePreference("extraConfirmations", v)}
            ...
            <div className="flex items-center justify-between rounded-xl border-2 border-border p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <Label htmlFor="reminders" className="text-lg font-medium text-foreground cursor-pointer">
                    Lembretes e notificações
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    Receber avisos de tarefas e compromissos
                  </p>
                </div>
              </div>
              <Switch
                ...
                onCheckedChange={(v) => updatePreference("remindersEnabled", v)}
                className="h-8 w-14 [&>span]:h-7 [&>span]:w-7 data-[state=checked]:[&>span]:translate-x-6"
              />
            </div>
            ...
            <div className="flex items-center justify-between rounded-xl border-2 border-border p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <Label htmlFor="sound" className="text-lg font-medium text-foreground cursor-pointer">
                    Alertas sonoros
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    Sons ao concluir tarefas e receber lembretes
                  </p>
                </div>
              </div>
              <Switch
